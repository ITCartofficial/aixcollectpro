...
const PopupMenu: React.FC<PopupMenuProps> = ({
    ...
    return (
        <>
            ...
            <div
                ref={popupRef}
                className="fixed z-50 bg-white border border-gray-200 rounded-lg shadow-lg py-2 min-w-[200px] max-h-[300px] overflow-y-auto"
                style={{
            ...
            >
                {menuItems.map((item) => (
                    <button
                        key={item.action}
                        className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 flex items-center transition-colors duration-150 cursor-pointer border-b border-neutral-300 last:border-b-0"
                        onClick={() => {
