...
const Login: React.FC = () => {
  ...
  const handleOtpVerified = () => {
    ...
    const tfaType = currentUser.twoFactorType || "none";
    if (tfaType === "none") {
  ...
  // Main render logic
  if (step === "dashboard") {
    window.location.href = "/"; // or use navigate("/dashboard") with react-router
    return null;
  }

  return (
    <>
      {/* Main Login Screen - Always visible as background */}
      <div className="min-h-screen flex bg-[#f6f7fa]">
        {/* Left Panel - Welcome Section */}
        <div className="flex-1 flex flex-col justify-center items-center bg-blue-600 rounded-l-[12px] px-8 py-8">
          <div className="flex flex-col items-center w-full">
